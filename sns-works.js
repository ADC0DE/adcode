/* SNS works (design-sns.html) — mag-channels.js 와 동일 패턴
...
(function () {
  ...
  const section = document.querySelector('.sns-works');
  const swiperRoot = document.querySelector('.sns-works-swiper');
  const marquee = document.querySelector('.sns-works-marquee');
  if (!section || !swiperRoot || !marquee) return;

  const sourceSlides = [...swiperRoot.querySelectorAll('.sns-card')];
  const tracks = [
    marquee.querySelector('[data-sns-track="0"]'),
    marquee.querySelector('[data-sns-track="1"]'),
  ];
